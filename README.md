...
./repo2docker /path/to/repo --name my-image --output /custom/path
```

## CLI Wrapper Usage

The `./repo2docker` script is a convenience wrapper that handles Docker operations:

```bash
./repo2docker <repo_path> [OPTIONS]
```
...
- `-t, --tag TAG` - Docker image tag (default: `repo2docker-image:latest`)
...
./repo2docker /path/to/repo --name my-image --output ~/outputs
```
